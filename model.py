...
def u_block_latent(x, latent, filters, training=False):
    with tf.variable_scope('layer_1'):
        ...
        x = tf.layers.batch_normalization(x, center=False, scale=False, training=training)
        x = ada_in(x, latent)
        x = tf.nn.relu(x)
...
def u_net(x, latent, name='u-net', training=False):
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope('conv_block'):
            x = u_block_latent(x, latent, 1028, training=training)
        ...
        for idx, (filters, skip_connection) in enumerate(zip(depth, skip_connections)):
            with tf.variable_scope(f'up_block{idx}'):
                ...
                x = u_block_latent(x, latent, filters, training=training)
...
def generator(x, latent, name, dtype=tf.float32, training=False):
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE, dtype=dtype):
        latent = generator_mapping(latent, training=training)
        x = u_net(x, latent, training=training)
...
def discriminator(x, laten_size, name, dtype=tf.float32, training=False):
    ...
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope('conv_adv'):
            x_adv = tf.layers.conv2d(inputs=x, filters=1,
                                     kernel_size=1, strides=1, padding='SAME')

        with tf.variable_scope('conv_cls'):
            x = tf.layers.flatten(x)
            x = tf.layers.dense(inputs=x, units=512)
            x = tf.cast(x, dtype=tf.float32)
            x = tf.layers.batch_normalization(x, training=training)
            x = tf.cast(x, dtype=tf.float16)
            x = tf.nn.leaky_relu(x)
            x_cls = tf.layers.dense(inputs=x, units=laten_size)
